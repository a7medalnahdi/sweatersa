(() => {
  ...
  function templateModal(){
    ...
    const btn=document.createElement('button');btn.className='sw-admin-templates-btn';btn.setAttribute('onclick','window.SweaterReadyTemplateActions?.open()');btn.innerHTML=`<i class="fa-solid fa-boxes-stacked"></i><span>قوالب جاهزة</span><b>${list.length}</b>`;actions.prepend(btn);
    const modal=document.createElement('div');modal.className='sw-admin-templates-modal';modal.innerHTML=`<div class="sw-admin-template-card"><header><div><strong>قوالب جاهزة</strong><span>قوالب معتمدة وجاهزة للاستخدام</span></div><button aria-label="إغلاق" onclick="window.SweaterReadyTemplateActions?.close()"><i class="fa-solid fa-xmark"></i></button></header><div class="sw-admin-template-grid">${list.map(t=>`<article><div class="sw-admin-template-preview">${t.preview?`<img src="${escape(t.preview)}" alt="">`:'<i class="fa-solid fa-layer-group"></i>'}</div><div><strong>${escape(t.name)}</strong><span>${escape(t.category||'عام')}</span></div><div class="sw-admin-template-actions"><button data-admin-template="${escape(t.id)}" onclick="window.SweaterReadyTemplateActions?.use(this.dataset.adminTemplate)">استخدام</button>${canEdit?`<button class="edit" data-edit-ready-template="${escape(t.id)}" onclick="window.SweaterReadyTemplateActions?.edit(this.dataset.editReadyTemplate)">تعديل</button>`:''}</div></article>`).join('')}</div></div>`;document.body.append(modal);
