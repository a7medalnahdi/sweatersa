(() => {
  ...
  function templateModal(){
    ...
    const btn=document.createElement('button');btn.className='sw-admin-templates-btn';btn.innerHTML=`<i class="fa-solid fa-boxes-stacked"></i><span>قوالب جاهزة</span><b>${list.length}</b>`;actions.prepend(btn);
    const modal=document.createElement('div');modal.className='sw-admin-templates-modal';modal.innerHTML=`<div class="sw-admin-template-card"><header><div><strong>قوالب جاهزة</strong><span>قوالب معتمدة وجاهزة للاستخدام</span></div><button aria-label="إغلاق"><i class="fa-solid fa-xmark"></i></button></header><div class="sw-admin-template-grid">${list.map(t=>`<article><div class="sw-admin-template-preview">${t.preview?`<img src="${escape(t.preview)}" alt="">`:'<i class="fa-solid fa-layer-group"></i>'}</div><div><strong>${escape(t.name)}</strong><span>${escape(t.category||'عام')}</span></div><div class="sw-admin-template-actions"><button data-admin-template="${escape(t.id)}">استخدام</button>${canEdit?`<button class="edit" data-edit-ready-template="${escape(t.id)}">تعديل</button>`:''}</div></article>`).join('')}</div></div>`;document.body.append(modal);
    const openModal=()=>{modal.classList.add('open');modal.style.display='grid'};
    const closeModal=()=>{modal.classList.remove('open');modal.style.removeProperty('display')};
    btn.addEventListener('click',openModal);modal.querySelector('header button').addEventListener('click',closeModal);modal.addEventListener('click',e=>{if(e.target===modal)closeModal()});
    modal.querySelectorAll('[data-admin-template]').forEach(button=>button.addEventListener('click',()=>{const template=list.find(t=>t.id===button.dataset.adminTemplate);if(!template)return;localStorage.setItem('sweaterPendingAdminTemplate',JSON.stringify(template));if(adapters[toolId])adapters[toolId](template);else window.dispatchEvent(new CustomEvent('sweater:apply-admin-template',{detail:template}));closeModal()}));
    modal.querySelectorAll('[data-edit-ready-template]').forEach(button=>button.addEventListener('click',()=>{const template=list.find(t=>t.id===button.dataset.editReadyTemplate);if(!template)return;window.dispatchEvent(new CustomEvent('sweater:edit-ready-template',{detail:template}));closeModal()}));
  }
