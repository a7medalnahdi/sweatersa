(() => {
  ...
  function templateModal(){
    ...
    modal.querySelectorAll('[data-edit-ready-template]').forEach(button=>button.onclick=()=>{const template=list.find(t=>t.id===button.dataset.editReadyTemplate);if(!template)return;window.dispatchEvent(new CustomEvent('sweater:edit-ready-template',{detail:template}));modal.classList.remove('open')});
  }
